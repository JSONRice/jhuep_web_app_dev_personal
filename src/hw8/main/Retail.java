...
public class Retail extends Application implements Initializable {
	...
	/* Preserves the added tuples of the customers, employees, and merchandise. */
	private ArrayList<RetailTuple> customerTuples;
	private ArrayList<RetailTuple> employeeTuples;
	private ArrayList<RetailTuple> merchandiseTuples;
	
	/**
	 * @author jsnrice
	 * @desc Private inner class to represent the tuple rows of the retail table.
	 * Columns are generic and go off an indexed array count from 0..8 In some cases
	 * not every column will be populated. This is only the case for the Merchandise data.
	 * For Merchandise a special three attribute tuple constructor has already been established.
	 */
	...
	public void popupPersonAddNew(final String titleStr, 
			boolean isCustomer, boolean isEmployee){
		Logger.getLogger(Retail.class.getName()).log(Level.INFO,"EVENT: popupPersonAddNew");
		...
		ok.setOnAction(new EventHandler<ActionEvent>() {			
			...
			public void handle(ActionEvent event) {
				...
				RetailTuple rt = new RetailTuple(fname.getText(), 
						lname.getText(), streetAddress.getText(), city.getText(),
						statesMenu.getSelectionModel().getSelectedItem(),
						zipcode.getText(), genderStr);
				if (isCustomer){
					customerTuples.add(rt);
				}
				else if (isEmployee){
					employeeTuples.add(rt); 
				}				
            	newStage.close();
			}
		});
	 ...
	 * @param isCustomer Tuples (rows) to insert are customers.
	 * @param isEmployee Tuples (rows) to insert are employees.
	 * @param isMerchandise Tuples (rows) to insert are merchandise.
	 */
	public void updateRetailTable(boolean isCustomer, 
			boolean isEmployee, boolean isMerchandise){		
		Logger.getLogger(Retail.class.getName()).log(Level.INFO,"EVENT: updateRetailTable");
	    ...
	    if (isMerchandise){
			Logger.getLogger(Retail.class.getName()).log(
					Level.INFO,"EVENT: updateRetailTable - adding merchandise tuples");
	    	for (RetailTuple rt : merchandiseTuples){
	    		retailRows.add(rt);
	    	}
	    }
	    else if (isCustomer){
	    	for (RetailTuple rt : customerTuples){
	    		retailRows.add(rt);
	    	}
	    }
	    else if (isEmployee){
	    	for (RetailTuple rt : employeeTuples){
	    		retailRows.add(rt);
	    	}
	    }
		...
		Logger.getLogger(Retail.class.getName()).log(Level.INFO,"EVENT: updateRetailTable - inserting tuples into retail table.");
	...
	public void popupMerchandiseAddNew(final String titleStr){
		Logger.getLogger(Retail.class.getName()).log(Level.INFO,"EVENT: popupMerchandiseAddNew");
		Stage newStage = new Stage();
		VBox comp = new VBox();
		...
		ok.setOnAction(new EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent event) {				
				RetailTuple rt = new RetailTuple(name.getText(),
						price.getText(), desc.getText());
				merchandiseTuples.add(rt);
            	newStage.close();
			}
		});
	...
	public void start(Stage primaryStage) throws Exception {
		try {
			...
			AnchorPane page = FXMLLoader.load(Retail.class.getResource("Retail.fxml"));
	...
	public void setEvents(){
		miexit.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent e) {
				Logger.getLogger(Retail.class.getName()).log(Level.INFO,"Shutting down clean exit.");
				System.exit(0);
			}
		});
		...
		customerAddNew.setOnAction(new EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent e) {
				Logger.getLogger(Retail.class.getName()).log(Level.INFO,"EVENT: customerAddNew.");
		...
		customerListAll.setOnAction(new EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent e) {
				Logger.getLogger(Retail.class.getName()).log(Level.INFO,"EVENT: customerListAll.");
		...
		employeeAddNew.setOnAction(new EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent e) {
				Logger.getLogger(Retail.class.getName()).log(Level.INFO,"EVENT: employeeAddNew.");
		...
		employeeListAll.setOnAction(new EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent e) {
				Logger.getLogger(Retail.class.getName()).log(Level.INFO,"EVENT: employeeListAll.");
		...
		merchandiseAddNew.setOnAction(new EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent e) {
				Logger.getLogger(Retail.class.getName()).log(Level.INFO,"EVENT: merchandiseAddNew.");
		...
		merchandiseListAll.setOnAction(new EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent e) {
				Logger.getLogger(Retail.class.getName()).log(Level.INFO,"EVENT: merchandiseListAll.");
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		customerTuples = new ArrayList<RetailTuple>();
		employeeTuples = new ArrayList<RetailTuple>();
		merchandiseTuples = new ArrayList<RetailTuple>();
